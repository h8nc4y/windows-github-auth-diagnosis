...
param(
    ...
    # 実運用の scan-wide 上限は 120 秒から延長できない。self-test は
    # lower-only の期限で、最終報告を含む fail-closed 経路を短時間に検証する。
    # binder固有のraw診断を避けるためobjectで受け、固定境界内で検証する。
...
$script:privateMarkerIntegrityFailureReported = $false
...
function Stop-PrivateMarkerIntegrityFailure {
    ...
    # Git/provider の raw 診断や絶対 path を再出力しない。曖昧な repository
    # 境界は、呼出元が機械判定できる固定診断と exit 2 へ統一する。
    # failure中のfinallyでも同じ境界が再失敗し得る。固定診断はprocess全体で
    # 一度だけ出し、二重framingやpathを含む後続例外へ移行させない。
    if (-not $script:privateMarkerIntegrityFailureReported) {
        [Console]::Error.WriteLine(
            "Private marker scan failed closed (integrity: $Reason)."
        )
        $script:privateMarkerIntegrityFailureReported = $true
    }
    exit 2
}

# deadline値の型・culture・範囲をscript内で確定し、parameter binderの
# path付きraw診断ではなく固定ASCII 1行とexit 2へ統一する。
$parsedScanDeadlineMilliseconds = 0
try {
    $scanDeadlineText = [string]$ScanDeadlineMilliseconds
    $scanDeadlineIsValid = [int]::TryParse(
        $scanDeadlineText,
    ...
    $scanDeadlineIsValid = $false
}
if (-not $scanDeadlineIsValid -or
...
try {
    ...
    Stop-PrivateMarkerIntegrityFailure -Reason 'scan-root-missing'
}
...
try {
    $hasProcessBoundary = Test-Path `
        -LiteralPath $processBoundary `
        -PathType Leaf `
        -ErrorAction Stop
}
catch {
...
try {
    ...
    # helper側の例外本文やInvocationInfoにはhelper絶対pathが含まれ得る。
    Stop-PrivateMarkerIntegrityFailure -Reason 'process-boundary'
}
...
# この公開skill自身だけを許可する。既存契約を広げず、関連repoへのURLも
# publish前の明示的な審査なしには通さない。
$allowedRepoUrlPattern = '^https://github\.com/h8nc4y/windows-github-auth-diagnosis(?:\.git)?$'
...
# 散文中の bare keyword は許可し、header 形式の長い token 候補だけを検出する。
# keyword 自体は分割し、scanner source が自己検出されない契約を維持する。
...
# 公開docsで使う予約domainとnoreply形式だけを許可し、実在風addressは検出する。
...
function Assert-HealthyGitBoundary {
    ...
    if ($Result.TimedOut -or
        ...
        -not $Result.ContainmentEstablished) {
        Stop-PrivateMarkerIntegrityFailure -Reason 'process-boundary'
    }
...
function Remove-PrivateMarkerGitIsolationRoot {
    ...
    try {
        if ($TestBoundaryFailure -eq 'isolation-remove') {
            throw 'Synthetic isolation cleanup failure.'
        }
        if (Test-Path `
        ...
        # temp rootやprovider診断を外へ出さず、cleanup不能も成功扱いしない。
        Stop-PrivateMarkerIntegrityFailure -Reason 'process-boundary'
    }
}
...
function Test-GitMarkerInAncestry {
    ...
    while ($null -ne $directory) {
        ...
        try {
            ...
            # 祖先の列挙不能は「metadataなし」とみなせない。raw provider
            # error を漏らさず、Git probe と同じ固定契約で fail closed にする。
...
if (-not [string]::IsNullOrWhiteSpace($environmentMarkers)) {
    Invoke-BoundedLineAction `
        ...
        -Context 'WINDOWS_GITHUB_AUTH_DIAGNOSIS_PRIVATE_MARKERS' `
        -Action {
            param($line, $lineNumber)
            Add-LocalMarker -Marker $line
        }
...
if ($null -eq $gitExe) {
    ...
    try {
        if ($TestBoundaryFailure -eq 'isolation-create') {
            throw 'Synthetic isolation creation failure.'
        }
        $gitIsolationRoot = Join-Path (
            [System.IO.Path]::GetTempPath()
        ) ("windows-github-auth-diagnosis-git-" + [System.Guid]::NewGuid().ToString('N'))
        New-Item `
            -ItemType Directory `
    ...
    try {
        ...
        if ($rootProbe.ExitCode -ne 0) {
            # root/ancestor の `.git` marker があるのに repository probe が
            # 成功しない状態は、壊れたmetadataやgitfileを含む曖昧な境界である。
            # Git stderr を解析・転記する前に固定 exit 2 で拒否する。
            if (Test-GitMarkerInAncestry) {
                Stop-PrivateMarkerIntegrityFailure -Reason 'git-probe'
            }
        ...
        if (-not $verifyGitIndexAtScanEnd -and
            -not [string]::IsNullOrWhiteSpace($gitIsolationRoot)) {
...
if ($findings.Count -gt 0) {
    # prefix/header/rowを明示LFの単一payloadへ積み、actual UTF-8 byte数で
    # 上限判定してから一度だけ出す。platform newline差とpartial tableを防ぐ。
    ...
    [byte[]]$outputBytes = [System.Text.Encoding]::UTF8.GetBytes(
    ...
    try {
        # serialize と stream取得の後、write直前に共通時計を再確認する。
        Assert-PrivateMarkerScanDeadline
...
# clean 判定も emit 直前に再確認し、最終probe後の期限超過を成功にしない。
Assert-PrivateMarkerScanDeadline
