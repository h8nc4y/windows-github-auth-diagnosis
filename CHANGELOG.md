...
- Upgrade the validation workflow to the immutable `actions/checkout` v7.0.1 commit (Node.js 24), disable checkout credential persistence, and retain the moving runner labels only where the jobs depend on standard `pwsh`, `git`, and native process primitives.
- Extend the exact workflow validator through checkout `with` mappings, bind nested safety keys to their active parent, and require YAML-valid whitespace before inline comments so invalid nesting or a scalar suffix cannot be truncated into an approved pin or safety value.
- Isolate each scanner self-test invocation behind its own temporary root and scope cleanup auditing to that owned root, so concurrent PowerShell hosts do not misclassify each other's same-prefix Git isolation directories as leaked artifacts.
...
- Add finite Windows, Ubuntu 24.04, and macOS job timeouts; run the scanner
  self-test under PowerShell 7 and Windows PowerShell 5.1; and verify the native
  POSIX `setsid(2)` fallback on macOS. The workflow initially used a reviewed
  immutable v5 checkout pin; T-014 / PR #30 later superseded it with immutable
  v7.0.1 and disabled checkout credential persistence.
  Every workflow job/step/key is validated against its owning job block.
  Quoted or flow-style extra jobs, quoted top-level keys, and active lines at
  unconsumed indentation are rejected.
