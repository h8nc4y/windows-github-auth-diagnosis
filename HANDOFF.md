...
- **T-015 closeout の文書同期も完了**。PR #34をsource commit `e070b49`、merge commit `a07670d`でmergeし、PR #35をsource commit `c98e524`、merge commit `71cddcb`でmergeした。続くPR #36をsource commit `c1328e8`、merge commit `5bc72cb`でmergeし、release gateの現況表現を同期した。
- `71cddcb`と`5bc72cb`は、2026/07/31までに観測したGit履歴を示す。どちらもownerが承認したT-006 release targetではない。着手時はGit / GitHubを再確認し、正本文書に記録されたSHAを現行`main`とみなさない。

## 次の一手

1. **T-006 GitHub Release v0.2.0（ゲート①）**: owner の4点承認（version / exact target commit / 公開タイミング / notes本文）が揃うまで tag push / Release 作成をしない。承認依頼の直前に`origin/main`のexact SHAを取得してtarget案とする。公開直前に再取得したSHAが承認済みtargetと一致しなければ停止し、targetの再承認を得る。
...
- T-015 closeout: PR #34 / #35 / #36はMERGED。`71cddcb`のmain push Validate run 30517452697、`c1328e8`のPR current-head run 30585919772、`5bc72cb`のmain push run 30586778028はWindows / Ubuntu / macOSがすべてsuccess。
