...
- 例外は次の2つだけ:
  1. **フロントのビジュアルデザインの「創出」はしない**（§11/§12）。配色・書体・レイアウトを新規に決める必要が出たら、ブリーフを書いて**停止**し、人間に渡す。
  2. **4ゲート**（§10: デプロイ/Actions/release・tag、課金/有料API、secret・実素材・実データの外部送信、製品要件の変更）に触れる操作は、実行/マージの直前で**停止して人間承認を取る**。
...
- **承認待ちをしない**: 4ゲート（§10）とフロントデザイン創出（§11）以外は、確認のために手を止めない。判断はあなたが下す。迷ったら「最小で安全な一歩」を選び、根拠を PR / コミットメッセージ / 開発ログ報告（§14）に残す。
...
- **デザイン委譲先の呼称統一**: 本ブリーフで「ClaudeDesign」と書いた箇所はすべて **Claude の `frontend-design` skill（人手仲介で渡す窓口）** の言い換え。別ツールではない。
...
## §11. フロントのビジュアルデザイン運用（Claude の frontend-design skill へブリーフ渡し）

- 現状このリポジトリに Web フロントエンドは無い。ただし配布・普及フェーズで**ランディングページ／ドキュメントサイト／バッジ以上の視覚要素**等を作る場面が出たら、本ルールを適用する。
- **Codex は配色・書体・レイアウトの「創出」をしない**。UI 実装（マークアップ、コンポーネント結線、配線、a11y のコード反映、ビルド）は担当してよいが、**ビジュアルデザインの方針決定は人手仲介**で行う。
- 手順: 視覚デザインが必要 → **§12 のブリーフを書いて停止** → 人間が Claude（`frontend-design` skill）に渡す → 返ってきた方針（color 4–6 hex / display+body type / layout / signature 要素）に**沿って** Codex が実装する。
- 機能だけの UI（純粋なロジック/配線で、新しい視覚言語の創出を伴わない）は自走で可。創出が混ざるなら停止。

---

## §12. フロントエンド・デザインブリーフ雛形

> 視覚デザインが必要になったら、以下を埋めて出力し**停止**する。人間がこれを Claude(frontend-design) に渡す。
...
## 5. Claude(frontend-design) への依頼事項（方針の創出）
- 出してほしい成果物: color 4–6 hex / display+body typeface / layout システム / signature 要素
- 参考にしたい雰囲気（言語化、URL 等は人間が補足）:

## 6. 受領後に Codex がやること（実装範囲の確認）
- 実装対象ファイル/ディレクトリ:
- 検証方法（ビルド/プレビュー/スクショ）:
...
- あなたはサンドボックスのため外部の開発ログ Vault には書けない。代わりに、**各セッションの応答末尾に日本語の「## 開発ログ報告（Obsidian用）」ブロック**を必ず出力する。人間（司令塔=Claude）がそれを Vault へ転記する。ブロックには以下を含める:
